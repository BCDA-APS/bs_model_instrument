#!/usr/bin/env python3
"""
Script to create a new instrument based on the 'src/bits/demo_instrument' template folder.
...
the pyproject.toml file with an entry under [tool.instruments] reflecting the new instrument's relative path.

The script includes a version flag and can be executed directly without needing to install the bits package.
"""
...
def main() -> None:
    ...
    This function parses command-line arguments (including a --version flag) to ensure the script 
    can be run standalone without needing to install the entire bits package.
    Validates that the new instrument name is a valid Python package name (lowercase letters, 
    ...
    parser.add_argument(
        "name",
        type=str,
        help="Name of the new instrument (this will be used as the new directory name and must be a valid package name)."
    )
    parser.add_argument(
        "--template",
        type=str,
        default="src/bits/demo_instrument",
        help="Path to the template instrument directory (default: src/bits/demo_instrument)."
    )
    parser.add_argument(
        "--dest",
        type=str,
    ...
    if re.fullmatch(r"[a-z][_a-z0-9]*", args.name) is None:
        logging.error("Invalid instrument name '%s'. Please use a valid Python package name "
                      "(lowercase letters, digits, and underscores, starting with a letter).", args.name)
        sys.exit(1)
