...
from apsbits.utils.delete_instrument import (
    delete_instrument,
...
from apsbits.utils.create_new_instrument import (
    copy_instrument,
    create_qserver,
    main as create_main,
)
...
@pytest.fixture
def mock_demo_dirs(tmp_path: Path, monkeypatch: "MonkeyPatch") -> Generator[tuple[Path, Path], None, None]:
    """
    Create mock demo directories for testing the create_new_instrument script.

    :param tmp_path: Pytest fixture providing a temporary directory.
    :param monkeypatch: Pytest fixture for patching.
    :yield: A tuple containing the mock demo instrument directory path and mock demo qserver directory path.
    """
    ...
    (demo_qserver_dir / "qs-config.yml").write_text("startup_module: demo_instrument.startup")
    ...
    monkeypatch.setattr(
        ...
        lambda *args: Path(*args) if args[0] != __file__ else tmp_path / "create_new_instrument.py",
    )
...
def test_delete_main_invalid_name(capsys: "CaptureFixture[str]") -> None:
    ...
    assert "Error: Invalid instrument name" in captured.err
...
def test_delete_main_nonexistent_instrument(
    ...
    assert excinfo.value.code == 1
    captured = capsys.readouterr()
    assert "Error: Neither instrument 'nonexistent' nor its qserver configuration exist" in captured.err
...
def test_delete_main_successful_deletion(
    ...
    captured = capsys.readouterr()
    assert "Instrument 'test_instrument' and its qserver configuration have been deleted" in captured.out
    assert not instrument_dir.exists()
    assert not qserver_dir.exists()
...
def test_copy_instrument(tmp_path: Path, mock_demo_dirs: tuple[Path, Path]) -> None:
    ...
    assert (destination_dir / "startup.py").exists()
    assert (destination_dir / "startup.py").read_text() == "print('Demo startup')"
...
def test_create_qserver(tmp_path: Path, mock_demo_dirs: tuple[Path, Path]) -> None:
    ...
    assert "python -m new_instrument.startup" in script_content
...
def test_create_main_successful_creation(
    ...
    mocker.patch("apsbits.utils.create_new_instrument.copy_instrument")
    mocker.patch("apsbits.utils.create_new_instrument.create_qserver")
    
    create_main()
    
    # Verify the functions were called with the correct arguments
    mocker.patch("apsbits.utils.create_new_instrument.copy_instrument").assert_called_once_with(
        tmp_path / "src" / "new_instrument"
    )
    mocker.patch("apsbits.utils.create_new_instrument.create_qserver").assert_called_once_with(
