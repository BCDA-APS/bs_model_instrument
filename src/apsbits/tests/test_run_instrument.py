...
from apsbits.utils.run_instrument import main, run_instrument_startup
...
def test_run_instrument_startup_no_main(mock_registry: MagicMock) -> None:
    """
    Test the run_instrument_startup function with a startup module without a main function.
...
def test_main_success(
    ...
    with patch("apsbits.utils.run_instrument.run_instrument_startup") as mock_run:
        ...
        monkeypatch.setattr(
            ...
            lambda _: type("Args", (), {"package_name": "test_package", "output": None})(),
        )
...
def test_main_success_with_output(
    mock_registry: MagicMock, monkeypatch: "MonkeyPatch", capsys: "CaptureFixture[str]", tmp_path: Path
) -> None:
    """
    ...
    with patch("apsbits.utils.run_instrument.run_instrument_startup") as mock_run:
        ...
        monkeypatch.setattr(
            "argparse.ArgumentParser.parse_args",
            lambda _: type("Args", (), {"package_name": "test_package", "output": str(output_file)})(),
        )
        ...
        assert output_file.read_text().count("device1") == 2
        assert output_file.read_text().count("device2") == 2
